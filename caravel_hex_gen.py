...
class CaravelHexGenerator(Generator):
    def run(self):
        ...
        #Store relative path from the caravel core root to the testcase
        testcase_root = os.path.join('verilog', 'dv', 'caravel', 'mgmt_soc', testcase)
        ...
        #Copy the compiled hex file from the build directory to the root directory of the generated core
        shutil.copy2(os.path.join(cwd, testcase_hex), testcase_hex)

        #Register the hex file in a fileset of the generated core and use copyto,
        #so that it ends up in the EDA tool work_root
